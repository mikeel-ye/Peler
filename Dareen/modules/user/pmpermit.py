...
from Dareen import TEMP_SETTINGS
from Dareen.helpers.adminHelpers import DEVS
from Dareen.helpers.basic import edit_or_reply
from Dareen.helpers.SQL.globals import addgvar, gvarstatus
from Dareen.helpers.tools import get_arg
...
async def incomingpm(client: Client, message: Message):
    try:
        from Dareen.helpers.SQL.globals import gvarstatus
        from Dareen.helpers.SQL.pm_permit_sql import is_approved
...
async def auto_accept(client, message):
    try:
        from Dareen.helpers.SQL.pm_permit_sql import approve, is_approved
    except BaseException:
        pass
...
async def approvepm(client: Client, message: Message):
    try:
        from Dareen.helpers.SQL.pm_permit_sql import approve
    except BaseException:
...
async def disapprovepm(client: Client, message: Message):
    try:
        from Dareen.helpers.SQL.pm_permit_sql import dissprove
    except BaseException:
        await message.edit("Running on Non-SQL mode!")
        return
...
async def setpm_limit(client: Client, cust_msg: Message):
    ...
    try:
        from Dareen.helpers.SQL.globals import addgvar
    ...
    Dareen = await cust_msg.edit("`Processing...`")
    if input_str and not input_str.isnumeric():
        return await Dareen.edit("**Harap masukan angka untuk PM_LIMIT.**")
    addgvar("PM_LIMIT", input_str)
    await Dareen.edit(f"**Set PM limit to** `{input_str}`")
...
async def stpmpt(client: Client, cust_msg: Message):
    ...
    try:
        import Dareen.helpers.SQL.globals as sql
    except AttributeError:
        await cust_msg.edit("**Running on Non-SQL mode!**")
        return
    Dareen = await cust_msg.edit("`Processing...`")
    ...
    if not message:
        return await Dareen.edit("**Mohon Reply Ke Pesan**")
    msg = message.text
    sql.addgvar("unapproved_msg", msg)
    await Dareen.edit("**Pesan Berhasil Disimpan Ke Room Chat**")
...
async def gtpmprmt(client: Client, cust_msg: Message):
    ...
    custom_message = sql.gvarstatus("unapproved_msg")
    if custom_message is not None:
        await Dareem.edit("**Pesan PMPERMIT Yang Sekarang:**" f"\n\n{custom_message}")
    else:
        await Dareen.edit(
...
async def gtpmprmt(client: Client, cust_msg: Message):
    ...
    if custom_message is None:
        await Dareen.edit("**Pesan PMPERMIT Anda Sudah Default**")
    else:
        sql.delgvar("unapproved_msg")
        await Dareen.edit("**Berhasil Mengubah Pesan Custom PMPERMIT menjadi Default**")
