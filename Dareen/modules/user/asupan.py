...
async def asupan_cmd(client: Client, message: Message):
    Dareen = await edit_or_reply(message, "`Tunggu Lagi Cari Asupan Buat Lu...`")
    await gather(
        Dareen.delete(),
        client.send_video(
            message.chat.id,
...
async def bkp_cmd(client: Client, message: Message):
    Dareen = await edit_or_reply(message, "`Tunggu Lagi Cari Bahan Coli Buat Lu..`")
    await gather(
        Dareen.delete(),
        client.send_video(
            message.chat.id,
            choice(
                [
                    ...
                    async for bkp in client.search_messages(
                        "bokepdareen", filter=enums.MessagesFilter.VIDEO
                    )
                ]
            ),
            reply_to_message_id=ReplyCheck(message),
