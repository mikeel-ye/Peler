...
async def opengc(client: Client, message: Message):
    ...
    try:
        ...
        await Dareen.edit(args)
    except Exception as e:
        await Dareen.edit(f"**INFO:** `{e}`")
...
async def joinvc(client: Client, message: Message):
    ...
    if message.from_user.id != client.me.id:
        Dareen = await message.reply("`Otewee Naik OS...`")
    else:
        Dareen = await message.edit("`Processing...`")
    with suppress(ValueError):
        chat_id = int(chat_id)
    try:
        await client.group_call.start(chat_id)
    except Exception as e:
        return await Dareen.edit(f"**ERROR:** `{e}`")
    await Dareen.edit(f"❏ **Berhasil Naik Os Gece Jelek Lu Tod**\n└ **Chat ID:** `{chat_id}`")
    await sleep(5)
...
async def leavevc(client: Client, message: Message):
    chat_id = message.command[1] if len(message.command) > 1 else message.chat.id
    if message.from_user.id != client.me.id:
        Dareen = await message.reply("`Turun dulu gaess...`")
    else:
        Dareen = await message.edit("`Processing...`")
    with suppress(ValueError):
        chat_id = int(chat_id)
