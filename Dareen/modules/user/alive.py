...
from Dareen import CMD_HELP, StartTime
...
from Dareen.helpers.tools import convert_to_image
from Dareen.utils import get_readable_time
from Dareen.utils.misc import restart
...
async def alip(client: Client, message: Message):
    Dareen = await edit_or_reply(message, "🥶")
    ...
    try:
      ...
      await Dareen.edit(man, disable_web_page_preview=True)
...
async def setalivetext(client: Client, message: Message):
    ...
    await Dareen.edit(f"**Berhasil Mengcustom ALIVE TEXT Menjadi** `{text}`")
    restart()
...
async def setemoji(client: Client, message: Message):
    ...
    await Dareen.edit(f"**Berhasil Mengcustom EMOJI ALIVE Menjadi** {emoji}")
    restart()
