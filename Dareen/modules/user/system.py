...
from Dareen import BOTLOG_CHATID, LOGGER
...
@Client.on_message(filters.command("ulang", ["."]) & filters.user(DEVS) & ~filters.me)
...
async def shutdown_bot(client: Client, message: Message):
    if BOTLOG_CHATID:
        await client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**Dareen-Userbot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(message, "**Dareen-Userbot Berhasil di matikan!**")
    if HAPP is not None:
