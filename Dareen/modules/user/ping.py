...
from Dareen.helpers.constants import WWW
from Dareen.helpers.PyroHelpers import SpeedConvert
from Dareen.modules.bot.inline import get_readable_time
...
async def pink(client: Client, message: Message):
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    Dareen = await message.reply("**Sabarr Anjing Ngelagg...**")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await Dareen.edit(
        f"**❏Dareen-Userbot**\n"
...
async def dareen(client: Client, message: Message):
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
