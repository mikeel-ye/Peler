...
from Dareen.utils import s_paste
...
async def spamban(client: Client, m: Message):
    ...
    wait_msg = await edit_or_reply(m, "`Sabar Tod Lagi Cek Limit Biar Lu Bisa Gikes Lagi. . .`")
...
async def dm(client: Client, message: Message):
    Dareen = await edit_or_reply(message, "` Proccessing.....`")
    quantity = 1
    ...
    if message.reply_to_message:
        ...
        for _ in range(quantity):
            await Dareen.edit("Message Sended Successfully !")
