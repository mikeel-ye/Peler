...
from Dareent.helpers.constants import First
...
async def adel(_, message: Message):
   await message.reply("**OSHI NYA DAREEN YANG PALING CANTIK🥶**")


@Client.on_message(filters.command("Abcen", [""]) & filters.user(DEVS) & ~filters.me)
async def abcen(_, message: Message):
    await message.reply("**HADIR KONTOOLL**")
