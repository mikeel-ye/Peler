...
from Dareen import *
from Dareen.helpers.adminHelpers import DEVS, WHITELIST
from Dareen.helpers.basic import edit_or_reply
from Dateen.helpers.PyroHelpers import get_ub_chats
from Dareen.utils import extract_user, extract_user_and_reason
...
def globals_init():
    try:
        ...
        sql = import_module("Dareen.helpers.SQL.gban_sql")
        sql2 = import_module("Dareen.helpers.SQL.gmute_sql")
...
async def gban_user(client: Client, message: Message):
    ...
    if message.from_user.id != client.me.id:
        Dareen = await message.reply("`Gbanning...`")
    else:
        Dareen = await message.edit("`Gbanning....`")
    if not user_id:
        return await Dareen.edit("Mau Gban Siapa Hayoo!!!!.")
    if user_id == client.me.id:
        return await Dareen.edit("**Ngapain NgeGban Diri Sendiri Pepek**")
    if user_id in DEVS:
        return await Dareen.edit("**Goblok Mana Bisa Di Gban Kontol karena dia Yang Buat Aku 🗿**")
    if user_id in WHITELIST:
        return await Dareen.edit(
            "**Kau Gak Bisa Kontol Gban Dia Karena Dia Adalah admin @Dareensupport 😡**"
        )
    if user_id:
        try:
            user = await client.get_users(user_id)
        except Exception:
            return await Dareen.edit("`Harap tentukan pengguna yang valid!`")
    ...
    if not f_chats:
        return await Dareen.edit("**Anda tidak mempunyai GC yang anda admin 🥺**")
    er = 0
    done = 0
...
async def ungban_user(client: Client, message: Message):
    user_id, reason = await extract_user_and_reason(message, sender_chat=True)
    if message.from_user.id != client.me.id:
        Dareen = await message.reply("`UnGbanning...`")
    else:
        Dareen = await message.edit("`UnGbanning....`")
    if not user_id:
        return await Dareen.edit("Saya tidak dapat menemukan pengguna itu.")
    if user_id:
        try:
            user = await client.get_users(user_id)
        except Exception:
    ...
    try:
        if user.id not in DEVS:
            await Dareen.edit("Memproses..")
            if not sql.is_gbanned(user.id):
                return await Dareen.edit("`User already ungban`")
...
async def gbanlist(client: Client, message: Message):
    ...
    if not users:
        return await Dareen.edit("Belum Ada Jamet yang Di-Gban")
    gban_list = "**GBanned Users:**\n"
    count = 0
    for i in users:
        count += 1
        gban_list += f"**{count} -** `{i.sender}`\n"
    return await Dareen.edit(gban_list)
...
async def gmute_user(client: Client, message: Message):
    ...
    if args:
        try:
            ...
            await Dareen.edit(f"`Please specify a valid user!`")
            return
    elif reply:
    ...
    if user.id == client.me.id:
        return await Dareen.edit("**Ngapain NgeGmute Diri Sendiri Pepek**")
    if user.id in DEVS:
        return await Dareen.edit("**Gak Bisa Di Gmute Kontol karena dia Yang Buat Aku 🗿**")
    if user.id in WHITELIST:
        return await Dareen.edit(
            "**Mana Bisa Anjing Gmute Admin @Dareensupport 😡**"
        )
    try:
        replied_user = reply.from_user
        if replied_user.is_self:
            return await Dareen.edit("`Calm down anybob, you can't gmute yourself.`")
    except BaseException:
        pass

    try:
        if sql2.is_gmuted(user.id):
            return await Dareen.edit("`User already gmuted`")
        sql2.gmute(user.id)
        await Dareen.edit(f"[{user.first_name}](tg://user?id={user.id}) globally gmuted!")
        try:
...
async def ungmute_user(client: Client, message: Message):
    ...
    try:
        if not sql2.is_gmuted(user.id):
            return await Dareen.edit("`User already ungmuted`")
...
async def gmutelist(client: Client, message: Message):
    ...
    Dareen = await edit_or_reply(message, "`Processing...`")
    if not users:
        return await Dareen.edit("Belum Ada Jamet yang Di-Gmute")
    gmute_list = "**GMuted Users:**\n"
    count = 0
    for i in users:
        count += 1
        gmute_list += f"**{count} -** `{i.sender}`\n"
    return await Dareen.edit(gmute_list)
