...
from Dareen.helpers.tools import get_arg, get_text, resize_media
from Dareen.utils.tools import add_text_img, bash
...
async def kang(client: Client, message: Message):
    ...
    Dareen = await edit_or_reply(message, "`Boleh juga ni stickernya malingg ahh🤪...`")
    media_ = None
    emoji_ = None
    is_anim = False
    is_video = False
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            if not replied.sticker.file_name:
                await Dareen.edit("**Stiker tidak memiliki Nama!**")
                return
            ...
            await Dareen.edit("**File Tidak Didukung**")
            return
        media_ = await client.download_media(replied, file_name="Uputt/resources/")
    else:
        await Dareen.edit("**Silahkan Reply ke Media Foto/GIF/Sticker!**")
        return
    if media_:
        ...
        if exist is not False:
            ...
            await Dareen.edit("`Membuat Sticker Pack Baru`")
            try:
                await client.send_message("Stickers", cmd)
            except YouBlockedUser:
                await client.unblock_user("stickers")
